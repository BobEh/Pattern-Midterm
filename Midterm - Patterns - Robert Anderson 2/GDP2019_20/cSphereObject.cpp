...
std::string cSphereObject::getWeaponType()
{
	return "";
}

iObject* cSphereObject::getWeapon()
{
	return this->weapon;
}

bool cSphereObject::Update(iObject* closestRobot)
{
	return true;
}

int cSphereObject::getHealth()
{
	return this->health;
}

void cSphereObject::setHealth(int newHealth)
{
	this->health = newHealth;
}

cSphereObject::cSphereObject(int robotType, iObject* weapon, glm::vec3 objectPosition)
{
	this->_scale = 0.0f;
	this->health = 100;
	...
	if (robotType == 1 || robotType == 2 || robotType == 3)
	{
		this->weapon = new cLaser(weapon, objectPosition);
	}
	else if (robotType == 4 || robotType == 5 || robotType == 6)
	{
		this->weapon = new cBomb();
	}
	else if (robotType == 7 || robotType == 8 || robotType == 9)
	{
		this->weapon = new cBullets();
	}
	else if (robotType == 10)
	{
		this->weapon = new cRocket();
	}
	else
	{
		weapon = NULL;
