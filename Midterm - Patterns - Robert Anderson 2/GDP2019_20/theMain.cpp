...
glm::vec3 cameraEye = glm::vec3(150.0f, 338.0f, -302.0f);
...
std::vector<iObject*> g_vec_pRobotObjects;
std::vector<iObject*> g_vec_pWeaponObjects;
...
bool Update(iObject* shootingRobot, iObject* closestRobot)
{
	bool hit = false;
	glm::vec3 TargetToRobotVector = closestRobot->getPositionXYZ() - shootingRobot->getPositionXYZ();

	glm::vec3 DirectionToTarget = glm::normalize(TargetToRobotVector);

	glm::vec3 MovementPerStep = DirectionToTarget * 0.1f;

	int numberOfPoints = glm::length(closestRobot->getPositionXYZ() - shootingRobot->getPositionXYZ());

	glm::vec3 testPoint = shootingRobot->getPositionXYZ();
	bool hasLineOfSite = true;
	float positionXDif = (shootingRobot->getPositionXYZ().x - closestRobot->getPositionXYZ().x) / 35;
	float positionYDif = (shootingRobot->getPositionXYZ().y - closestRobot->getPositionXYZ().y) / 35;
	float positionZDif = (shootingRobot->getPositionXYZ().z - closestRobot->getPositionXYZ().z) / 35;
	glm::mat4 matModel = glm::mat4(1.0f);
	for (int k = 0; k < 35; k++)
	{
		/*cPhysics::sPhysicsTriangle closestTriangleArena;
		pPhsyics->GetClosestTriangleToPoint_FRAMEWORKS_AND_PATTERNS(testPoint, transformedMesh, closestTriangleArena);
		if (testPoint.y < closestTriangleArena.verts[0].y)
		{
			hasLineOfSite = false;
			break;
		}*/
		
		pDebugSphere->setPositionXYZ(glm::vec3(shootingRobot->getPositionXYZ().x - (positionXDif * k), shootingRobot->getPositionXYZ().y - (positionYDif * k), shootingRobot->getPositionXYZ().z - (positionZDif * k)));
		pDebugSphere->setScale(0.5f);
		if (shootingRobot->getWeapon()->getWeaponType() == "Laser")
		{
			pDebugSphere->setDebugColour(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f));
		}
		else if (shootingRobot->getWeapon()->getWeaponType() == "Bullets")
		{
			pDebugSphere->setDebugColour(glm::vec4(0.0f, 0.0f, 1.0f, 1.0f));
		}
		else if (shootingRobot->getWeapon()->getWeaponType() == "Bomb")
		{
			pDebugSphere->setDebugColour(glm::vec4(1.0f, 1.0f, 0.0f, 1.0f));
		}
		else if (shootingRobot->getWeapon()->getWeaponType() == "Rocket")
		{
			pDebugSphere->setDebugColour(glm::vec4(1.0f, 0.0f, 1.0f, 1.0f));
		}
		else
		{
		...
		pPhsyics->GetClosestTriangleToPoint(pDebugSphere->getPositionXYZ(), arenaMesh, closestPoint, closestTriangle);
		if (pDebugSphere->getPositionXYZ().y < closestPoint.y)
		{
			hasLineOfSite = false;
		}
	}
	if (hasLineOfSite)
	{
		if (shootingRobot->getWeapon()->getWeaponType() == "Laser")
		{
			closestRobot->setHealth(closestRobot->getHealth() - 4);
		}
		else if (shootingRobot->getWeapon()->getWeaponType() == "Bullets")
		{
			closestRobot->setHealth(closestRobot->getHealth() - 2);
		}
		else if (shootingRobot->getWeapon()->getWeaponType() == "Bomb")
		{
			closestRobot->setHealth(closestRobot->getHealth() - 25);
		}
		else if (shootingRobot->getWeapon()->getWeaponType() == "Rocket")
		{
			closestRobot->setHealth(closestRobot->getHealth() - 30);
		}
		hit = true;
	}
	else
	{
		float positionX = randInRange(-120.0f, 120.0f);

		float positionZ = randInRange(-120.0f, 120.0f);
		pPhsyics->GetClosestTriangleToPoint(glm::vec3(positionX, 50.0f, positionZ), arenaMesh, closestPoint, closestTriangle);
		float positionY = closestPoint.y + 0.5f;
		shootingRobot->setPositionXYZ(glm::vec3(positionX, positionY, positionZ));
		shootingRobot->setInverseMass(1.0f);
		hit = false;
	}
	return hit;
}
...
int main(void)
{
	...
	pTheModelLoader->LoadPlyModel("assets/models/Arena_Terrain_xyz_n.ply", arenaMesh);

	cMesh bunnyMesh;
	pTheModelLoader->LoadPlyModel("assets/models/bun_zipper_res3_XYZ_n.ply", bunnyMesh);

	cMesh boxMesh;
	pTheModelLoader->LoadPlyModel("assets/models/Cube_1_Unit_from_origin_XYZ_n.ply", boxMesh);
	...
	pTheVAOManager->LoadModelIntoVAO("arena",
		arenaMesh,		// Sphere mesh info
		sphereMeshInfo,
		shaderProgID);

	sModelDrawInfo bunnyMeshInfo;
	pTheVAOManager->LoadModelIntoVAO("bunny",
		bunnyMesh,		// Sphere mesh info
		sphereMeshInfo,
		shaderProgID);

	sModelDrawInfo boxMeshInfo;
	pTheVAOManager->LoadModelIntoVAO("box",
		boxMesh,		// Sphere mesh info
		sphereMeshInfo,
		shaderProgID);

	iObject* pBox = pFactory->CreateObject("mesh", 0, NULL, glm::vec3());
	pBox->setMeshName("box");
	pBox->setFriendlyName("box");	// We use to search 
	pBox->setPositionXYZ(glm::vec3(0.0f, 50.0f, 0.0f));
	pBox->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pBox->setScale(0.02f);
	pBox->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pBox->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pBox->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pBox->setInverseMass(1.0f);
	pBox->set_SPHERE_radius(1.0f);
	pBox->setIsVisible(true);
	pBox->setIsWireframe(false);

	// Sphere and cube
	iObject* pSphere = pFactory->CreateObject("sphere", 0, pBox, glm::vec3());
	...
	iObject* pArena = pFactory->CreateObject("mesh", 0, NULL, glm::vec3());
	pArena->setMeshName("arena");
	pArena->setFriendlyName("arena");	// We use to search 
	pArena->setPositionXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pArena->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pArena->setScale(1.0f);
	pArena->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pArena->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pArena->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pArena->setInverseMass(0.0f);
	pArena->setIsVisible(true);
	pArena->setIsWireframe(false);
	::g_vec_pGameObjects.push_back(pArena);

	iObject* pBunny = pFactory->CreateObject("sphere", 0, pBox, glm::vec3());
	pBunny->setMeshName("bunny");
	pBunny->setFriendlyName("bunny");	// We use to search 
	pBunny->setPositionXYZ(glm::vec3(0.0f, 50.0f, 0.0f));
	pBunny->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pBunny->setScale(1.0f);
	pBunny->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pBunny->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pBunny->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pBunny->setInverseMass(0.0f);
	pBunny->setIsVisible(true);
	pBunny->setIsWireframe(false);
	//::g_vec_pRobotObjects.push_back(pBunny);



	// Will be moved placed around the scene
	pDebugSphere = pFactory->CreateObject("sphere", 0, NULL, glm::vec3());
	...
	for (int i = 0; i < 20; i++)
	{
		...
		int robotType = randInRange(1, 10);
		float positionX = randInRange(-120.0f, 120.0f);
		
		float positionZ = randInRange(-120.0f, 120.0f);
		pPhsyics->GetClosestTriangleToPoint(glm::vec3(positionX, 50.0f, positionZ), arenaMesh, closestPoint, closestTriangle);
		float positionY = closestPoint.y + 0.5f;
		iObject* pRobot = pFactory->CreateObject("sphere", robotType, pBox, glm::vec3(positionX, positionY, positionZ));
		pRobot->setMeshName("bunny");
		pRobot->setFriendlyName("asteroid011");	// We use to search 
		pRobot->setPositionXYZ(glm::vec3(positionX, positionY, positionZ));
		pRobot->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
		pRobot->setScale(1.0f);
		pRobot->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
		pRobot->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
		pRobot->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
		pRobot->setInverseMass(1.0f);
		pRobot->set_SPHERE_radius(1.0f);
		pRobot->setIsVisible(true);
		pRobot->setIsWireframe(false);
		::g_vec_pRobotObjects.push_back(pRobot);
		iObject* pWeapon = pFactory->CreateObject("mesh", 0, NULL, glm::vec3());
		pWeapon->setMeshName("box");
		pWeapon->setFriendlyName("box");	// We use to search 
		pWeapon->setPositionXYZ(glm::vec3(positionX, positionY + 1.0f, positionZ));
		pWeapon->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
		pWeapon->setScale(0.5f);
		if (pRobot->getWeapon()->getWeaponType() == "Laser")
		{
			pWeapon->setObjectColourRGBA(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f));
		}
		else if (pRobot->getWeapon()->getWeaponType() == "Bomb")
		{
			pWeapon->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 0.0f, 1.0f));
		}
		else if (pRobot->getWeapon()->getWeaponType() == "Bullets")
		{
			pWeapon->setObjectColourRGBA(glm::vec4(0.0f, 0.0f, 1.0f, 1.0f));
		}
		else
		{
			pWeapon->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
		}
		pWeapon->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
		pWeapon->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
		pWeapon->setInverseMass(1.0f);
		pWeapon->set_SPHERE_radius(1.0f);
		pWeapon->setIsVisible(true);
		pWeapon->setIsWireframe(false);
		::g_vec_pWeaponObjects.push_back(pWeapon);
	}

	glm::mat4 matWorld = calculateWorldMatrix(pArena);

	pPhsyics->CalculateTransformedMesh(arenaMesh, matWorld, transformedMesh);
...
// View matrix
		...
		for (int index = 0; index != ::g_vec_pRobotObjects.size(); index++)
		{
			glm::mat4 matModel = glm::mat4(1.0f);

			iObject* pCurrentObject = ::g_vec_pRobotObjects[index];
		...
		//draw weapons
		for (int index = 0; index != ::g_vec_pWeaponObjects.size(); index++)
		{
			glm::mat4 matModel = glm::mat4(1.0f);

			iObject* pCurrentObject = ::g_vec_pWeaponObjects[index];
			pCurrentObject->setPositionXYZ(g_vec_pRobotObjects.at(index)->getPositionXYZ() + glm::vec3(0.0f,3.5f,0.0f));
		...
		pPhsyics->IntegrationStep(::g_vec_pRobotObjects, 0.05f);
		...
		for (int j = 0; j < ::g_vec_pRobotObjects.size(); j++)
		{
			...
			pPhsyics->GetClosestTriangleToPoint(g_vec_pRobotObjects.at(j)->getPositionXYZ(), transformedMesh, closestPoint, closestTriangle);
			...
			float distance = glm::length(g_vec_pRobotObjects.at(j)->getPositionXYZ() - closestPoint);
			bool hit = false;
			if (distance <= g_vec_pRobotObjects.at(j)->get_SPHERE_radius())
			{
				g_vec_pRobotObjects.at(j)->setInverseMass(0.0f);

				if (g_vec_pRobotObjects.at(j)->getHealth() > 0)
				{
					if (g_vec_pRobotObjects.at(j)->getWeapon()->getWeaponType() == "Laser" || g_vec_pRobotObjects.at(j)->getWeapon()->getWeaponType() == "Bullets")
					{
						iObject* closestRobot;
						if (j == g_vec_pRobotObjects.size() - 1)
						{
							closestRobot = g_vec_pRobotObjects.at(j - 1);
						}
						else
						{
							closestRobot = g_vec_pRobotObjects.at(j + 1);
						}
						for (int k = 0; k < g_vec_pRobotObjects.size(); k++)
						{
							float distance = glm::length(g_vec_pRobotObjects.at(j)->getPositionXYZ() - g_vec_pRobotObjects.at(k)->getPositionXYZ());
							if (k != j && distance < glm::length(g_vec_pRobotObjects.at(k)->getPositionXYZ() - closestRobot->getPositionXYZ()))
							{
								closestRobot = g_vec_pRobotObjects.at(k);
							}
						}
						while (!hit)
						{
							hit = Update(g_vec_pRobotObjects.at(j), closestRobot);
						}
					}
					else if (g_vec_pRobotObjects.at(j)->getWeapon()->getWeaponType() == "Bomb" || g_vec_pRobotObjects.at(j)->getWeapon()->getWeaponType() == "Rocket")
					{
						iObject* closestRobot;
						if (j == g_vec_pRobotObjects.size() - 1)
						{
						...
						Update(g_vec_pRobotObjects.at(j), closestRobot);
					}
				}
				else
				{
					g_vec_pRobotObjects.at(j)->setObjectColourRGBA(glm::vec4(0.2f, 0.2f, 0.2f, 1.0f));
				}
			}
			// A more general 
			pPhsyics->TestForCollisions(::g_vec_pGameObjects);
			pPhsyics->TestForCollisions(::g_vec_pRobotObjects);
			...
			glm::vec3 CentreToClosestPoint = g_vec_pRobotObjects.at(j)->getPositionXYZ() - closestPoint;

			// Direction that ball is going is normalized velocity
			glm::vec3 directionBall = glm::normalize(g_vec_pRobotObjects.at(j)->getVelocity());	// 1.0f
			...
			pDebugRenderer->addLine(g_vec_pRobotObjects.at(j)->getPositionXYZ(),
				closestPoint,
